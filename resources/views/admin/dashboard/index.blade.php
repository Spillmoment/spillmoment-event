...
<script>
    ...
    document.getElementById('label-count-unit2').innerHTML = `<a href="{{ route('unit.index') }}">${total_unit}
Agensi</a>`;
...
for (let loop = 0; loop < data_unit.length; loop++) { first_month=data_unit[0].monthname;
    last_month=data_unit[data_unit.length-1].monthname; count_last_month=data_unit[data_unit.length-1].count;
    count_last2_month=data_unit[data_unit.length-2].count; }
    document.getElementById('unit-month-data').innerHTML=`${first_month.substring(0,3)} -
    ${last_month.substring(0,3)}` // Count persent up or down from last month const test=()=> {
    let count_result = ''
    let devided_count = count_last_month > count_last2_month ? (count_last2_month / count_last_month) :
    (count_last_month / count_last2_month)
    ...
    } else if(count_last_month < count_last2_month) { count_result=`<span class="fas fa-angle-down text-danger"></span>
        ...
        <script>
            ...
            doc.addEventListener("DOMContentLoaded", function (event) {
                ...
                if (doc.querySelector('.ct-chart-sales-value')) {
                    ...
                    let list_months = ['January', 'February', 'March', 'April', 'May', 'June', 'July', 'August',
                ...
                if (doc.querySelector('.ct-chart-traffic-share')) {
                    ...
                    for (let index = 0; index < data_active_diactive.length; index++) {
                        ...
                        if (data_active_diactive[index].count === check_max) {
                            document.getElementById('max-percent').innerHTML =
                                `<span class="icon w-20 icon-xs icon-secondary mr-1"><span class="fas fa-check-double"></span></span> ${data_active_diactive[index].status == 1 ? 'Aktif' : null} <span class="h6">${data_active_diactive[index].count}</span>`
                        }
                        document.getElementById('min-percent').innerHTML =
                            `<span class="icon w-20 icon-xs icon-blue mr-1"><span class="fas fa-times-circle"></span></span> ${data_active_diactive[index].status == 0 ? 'Tidak Aktif' : null} <span class="h6">${data_active_diactive[index].count}</span>`
                    }
